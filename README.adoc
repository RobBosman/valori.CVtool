...
React + Redux + Kotlin + vert.x + MongoDB + Docker application to edit and maintain curriculum vitae, see https://cvtool.cerios.nl/[cvtool.cerios.nl].
...
2. Search for 'CVtool'
3. Under _Azure Active Directory_ select *CVtool application*
...
* Change the port number in `C:\Program Files\MongoDB\Server\*\bin\mongod.cfg`:
...
4. Copy `docker/letsencrypt_certs/*` to volume _letsencrypt_certs_. +
...
=== Build and publish docker images
 ...
 docker tag bransom/cvtool-backend:latest bransom/cvtool-backend:MSAL-v5
 docker tag bransom/cvtool-frontend:latest bransom/cvtool-frontend:MSAL-v5
 docker compose -f docker-compose.yaml --env-file=/secret/.env up -d
 docker system prune -f

 docker push bransom/cvtool-backend:MSAL-v5
 docker push bransom/cvtool-frontend:MSAL-v5
...
== Docker commands
 ...
 # Logs and backups
 docker logs "$(docker ps -aqf 'ancestor=bransom/cvtool-backend')" > ~rbosman/cvtool_be.log 2>&1
 docker logs "$(docker ps -aqf 'ancestor=bransom/cvtool-frontend')" > ~rbosman/cvtool_fe.log 2>&1
 cp -R /var/lib/docker/volumes/root_cvtool_backup/_data/2026-05-12 ~rbosman
 cp -R /var/lib/docker/volumes/root_mongodb_backup/_data/2026-05-12 ~rbosman
 rm -rf ~rbosman/cvtool*.log ~rbosman/2026-05-12
...
==== mongo shell
 ...
 db.audit_log.deleteMany({ timestamp: { $lt: "2025-01-01" } });
