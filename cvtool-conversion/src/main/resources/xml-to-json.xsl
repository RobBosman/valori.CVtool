...
<xsl:stylesheet
    ...
    <xsl:template match="cv:set_of__account">
        ...
        ,"education": [<xsl:apply-templates
            select="*/cv:cv/cv:opleiding[not(cv:soort_opleiding) or cv:soort_opleiding = 0]" mode="opleiding"/>]
    ...
    <xsl:template match="cv:cv">
        ...
        <xsl:if test="util:jsonInt(cv:persoonsgegevens/cv:werkervaring_sinds) > 0">
            ,"workingSince":
            <xsl:value-of select="util:jsonInt(cv:persoonsgegevens/cv:werkervaring_sinds)"/>
        </xsl:if>
        <xsl:if test="util:jsonInt(cv:persoonsgegevens/cv:it_ervaring_sinds) > 0">
            ,"inItSince":
            <xsl:value-of select="util:jsonInt(cv:persoonsgegevens/cv:it_ervaring_sinds)"/>
        </xsl:if>
        }
    </xsl:template>
    ...
    <xsl:template match="cv:opleiding" mode="opleiding">
        ...
        ,"institution": "
        <xsl:choose>
            <xsl:when test="contains(cv:naam_instituut, cv:plaats_instituut)">
                <xsl:value-of select="cv:naam_instituut"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="util:jsonText(concat(cv:naam_instituut, ' ', cv:plaats_instituut))"/>
            </xsl:otherwise>
        </xsl:choose>
        "
        ,"yearTo":
        <xsl:value-of select="util:jsonInt(cv:jaar_diploma)"/>
    ...
    <xsl:template match="cv:publicatie">
        ...
        <xsl:if test="$titel or $media">
            ...
            ,"cvId": "<xsl:value-of select="util:uuid(../@id)"/>"
            ,"year":
            <xsl:value-of select="util:jsonInt(cv:jaar)"/>
    ...
    <xsl:template match="cv:referentie">
        <xsl:if test="cv:naam_referent">
            ...
            ,"includeInCv":
            <xsl:apply-templates select="cv:opnemen_in_cv" mode="toBoolean"/>
            }
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="cv:talenkennis">
        ...
        <xsl:if test="($taal_NL or $taal_UK) and $mondeling">
            ...
            ,"skillLevel":
            <xsl:apply-templates select="$mondeling" mode="languageLevel"/>
            }
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="cv:branchekennis">
        ...
        <xsl:if test="$omschrijving_NL or $omschrijving_UK">
            ...
            ,"skillLevel":
            <xsl:value-of select="util:jsonLevel(cv:kennisniveau)"/>
            }
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="cv:werkopdracht">
        ...
        ,"sortIndex":
        <xsl:value-of select="util:jsonInt(cv:sort_index)"/>
        ,"includeInCv":
        <xsl:apply-templates select="cv:opnemen_in_cv" mode="toBoolean"/>
        }
    </xsl:template>
